-- Storage rows cannot be deleted from SQL ("Use the Storage API instead"), so instead of removing
-- a failed document we mark it discarded and release its SHA-256 fingerprint. The uploaded file
-- stays in the bucket for provenance; the retry creates a fresh document row.
  ...
  if p_delete_document then
    update public.documents
       set status = 'discarded',
           sha256 = null,
           metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object('discarded_at', now(), 'original_sha256', v_doc.sha256)
     where id = p_document;
  end if;

  insert into public.audit_logs (organization_id, actor_user_id, action, entity_type, entity_id, metadata)
  values (v_doc.organization_id, auth.uid(), 'document.discarded', 'document', p_document::text,
          jsonb_build_object('filename', v_doc.filename, 'invoices_removed', v_invoices, 'contracts_removed', v_contracts, 'released', p_delete_document));
